def first_fit(items, capacity=1.0):
    ...
    for item in items:
        ...
        # Create a new bin if item cannot fit
        if not placed:
...
def best_fit_decreasing(items, capacity=1.0):
    ...
    for item in sorted_items:
        ...
        # Find the bin that will have
        # the least remaining space after placing item
        for i, space in enumerate(bins):
...
# Lower bound = ceiling(total item size / bin capacity)
